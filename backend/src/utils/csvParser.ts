 ...
 */
export function validateColumns(headers: string[]): { valid: boolean; error?: string } {
 ...
 */
export function parseCSV(content: string | Buffer): ParseResult {
  try {
...
export function buildGraphData(
  transactions: Transaction[],
  accountScores: Record<string, { score: number; patterns: string[]; ring_id: string | null }>,
  fraudRingEdges: Set<string>
) {
  const nodeMap = new Map<string, { id: string; txCount: number }>();
